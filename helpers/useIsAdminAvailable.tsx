...
import { useAvailabilitySlotsQuery } from "./useAvailabilitySlotsQuery";
...
/**
 * A React hook to determine if an admin is currently available for live chat.
 * It fetches availability slots and compares them against the user's current time,
 * respecting the timezones specified in each slot.
 *
 * @returns An object containing:
 * - `isAvailable`: boolean - True if an admin is currently available.
 * - `isLoading`: boolean - True while fetching availability data.
 * - `nextAvailableSlot`: object | null - Details of the next upcoming slot if not currently available.
 */
export const useIsAdminAvailable = () => {
  const { data: slots, isLoading, isError } = useAvailabilitySlotsQuery();

  const availability = useMemo(() => {
    if (isLoading || isError || !slots) {
      return {
        isAvailable: false,
